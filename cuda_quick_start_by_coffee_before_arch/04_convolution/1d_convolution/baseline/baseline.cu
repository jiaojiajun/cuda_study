...
__global__ void convolution(float* array, float* kernel, float*res){
    int r = NKERNEL / 2;
...
int main(){
    ...
    cudaMemcpy(fpDeviceKernel, fpHostKernel, nKernelBytes, cudaMemcpyHostToDevice);

    int blockSize = 256 ;
    int gridSize = (NARRAY + blockSize - 1) /blockSize;

    convolution<<<gridSize, blockSize>>>(fpDeviceArray, fpDeviceKernel, fpDeviceRes);
